...
async def test_canonical_demo_execution(tmp_path):
    ...
    assert len(summary.rollback_summary.reverted_action_ids) == 1

    # Assert restored file state: config.json created in step 3 is deleted, restoring workspace to step 2
